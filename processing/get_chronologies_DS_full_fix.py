...
def main():
    ...
    for admission_id in tqdm(admission_ids):
        ...
        # ========== 关键改动：不再调用 get_last_day，保留全量时间 ==========
        formatted_full = format_result(combined_tl_rel)
        ...
        # 输出文件名：full_{modality}_{admission_id}.csv
        formatted_full.to_csv(f'{output_dir}/full_{modality}_{admission_id}.csv', index=False)
